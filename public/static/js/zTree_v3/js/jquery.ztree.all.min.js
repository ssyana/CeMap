...
$.fn.treeNodeSelector=function(options){
	var settings=$.extend({
		treeId:"treeDemo",
		treeSettings:{
	      check: {
	...
	/////////tree init/////////
	var htmTemp='<i class="icon_selectArrow"></i>'+'<input type="text" readonly value="" class="normal-form-text" />'+
            '<div class="treeList">'+
              '<ul id="'+settings.treeId+'" class="ztree"></ul>'+
            '</div>';
    $this.html(htmTemp);
    $.fn.zTree.init($("#"+settings.treeId), settings.treeSettings, settings.zNodes);
    var treeObj = $.fn.zTree.getZTreeObj(settings.treeId);
    treeObj.expandAll(false);
    //获取默认选中的项，赋值
    var nodes = treeObj.getCheckedNodes(true);
    console.log(nodes);
    if(nodes.length){
    	var cityObj = $this.find(".normal-form-text");
     	cityObj.attr("value", nodes[0].name);
    }
	///////////////////functions//////////////////

	function onClick(e, treeId, treeNode) {
      var zTree = $.fn.zTree.getZTreeObj(settings.treeId);
    ...
    $this.find(".normal-form-text,.icon_selectArrow").click(function(){
        $this.find(".treeList").show();
    });
...
$.fn.treeNodeSelectorMulti=function(options){
	var settings=$.extend({
		treeId:"treeDemo",
		treeSettings:{
	      check: {
